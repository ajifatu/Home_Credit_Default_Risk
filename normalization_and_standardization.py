...
def scaling(data:pd.DataFrame) -> pd.DataFrame:
    ...
    """
     Cette fonction permet de mettres les colonnes numériques d'un dataset 
     sur une meme echelle
    """
        ...
        if col not in ["SK_ID_CURR","SK_BUREAU_ID","SK_ID_PREV"]:
                ...
                if not (-0.5 <= data[col].skew() <=0.5):
                    if data.isna().sum().sum()== 0:
                        ...
                        data_final=pd.DataFrame(MinMaxScaler().fit_transform(data),columns=data.columns)
                        return data_final
                    else:
                        print("Le dataset contient des valeurs manquantes ")
                    
                else: 
                    data=replace_outliers_Zscore(data)
                    data_final=pd.DataFrame(StandardScaler().fit_transform(data),columns=data.columns)
...
def scaling_with_na_2(data: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    # si les données sont asymétriques
    # si les données sont symétriques